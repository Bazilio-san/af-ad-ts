...
module.exports = function search (server, _settings) {
  server.search('rootDSE', (req, res, _next) => {
    res.send({
      dn: '',
      attributes: {
        controls: ['foo'],
  ...
  server.search(baseDN, (req, res, _next) => {
    ...
    if (/^.member=cn=.*,(\s*)?ou=domain (users|admins),(\s*)?dc=domain/i.test(filter)) {
      const groups = schema.getByRDN(filter.replace(/member=/g, '')).attributes.memberOf;
      if (groups) {
        sendGroups(groups);
      }
      res.end();
      ...
      for (const k of keys) {
        ...
        g.value.attributes.memberOf.forEach((sg) => {
          if (sg.attributes.cn.toLowerCase().replace(/\s/g, '') === cn) {
            groups = groups ? [].concat(groups, [g.value]) : [g.value];
          }
        });
      }
